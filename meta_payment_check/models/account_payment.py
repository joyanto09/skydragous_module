...
class AccountPaymentCheck(models.Model):
    ...
    receive_check_date = fields.Date(string="Date")
    check_status = fields.Char(string="Status", compute="change_status")
    # <!--25-01-2020-->
    ...
    def change_status(self):
        for payment in self:
            ...
            elif payment.payment_type in ['transfer']:
                payment.check_status = 'Internal Transfer'
            else:
    ...
    def _pay_order_details(self):
        ...
        if self.check_pay_order == True:
            self.check_pay_order_details = "Issue PO to- "

        else:
             self.check_pay_order_details = " "  
        
            
    is_ac_payee_only = fields.Boolean(string='A/C Payee Only')        
