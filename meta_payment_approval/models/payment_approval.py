...
class AccountPaymentApproval(models.Model):
    ...
    def operation_send_approve(self):
        try:
            ...
            users.notify_success("Operator Sent a Payments Approval To Accounting")
            
            if not self.env['mail.channel'].search([('name','=','Payments Approval Notification')]):
                self.env['mail.channel'].create({'name': 'Payments Approval Notification','public':'public', 'group_ids': users_group, 'email_send': True})
            payments_message= self.env['mail.channel'].search([('name','=','Payments Approval Notification')])

            payments_message.message_post(
            subject='Operator Sent a Payments Approval',
            body='''Operator Sent a Payments Approval To Accounting''',
    ...
    def first_approval(self):
        try:
            users=self.env['res.users'].search([('active','=',True)])
            # ref = self.name
            users.notify_success("Accounting Approved a Payments")

            channel_id = self.env['mail.channel'].search([('name','=','Payments Approval Notification')])

            channel_id.message_post(
            subject='Accounting Approved a Payments',
            body='''Accounting Approved a Payments & Approval Sent To Finance ''',
            subtype='mail.mt_comment')
    ...
    def first_approval_reject(self):
        try:
            users=self.env['res.users'].search([('active','=',True)])
            # ref = self.name
            users.notify_info("Accounting Canceled a Payments")

            channel_id = self.env['mail.channel'].search([('name','=','Payments Approval Notification')])

            channel_id.message_post(
            subject='Accounting Canceled a Payments',
            body='''Accounting Canceled a Payments''',
            subtype='mail.mt_comment')
    ...
    def second_approval(self):
        try:
            users=self.env['res.users'].search([('active','=',True)])
            # ref = self.name
            users.notify_success("Finance Approved a Payments")

            channel_id = self.env['mail.channel'].search([('name','=','Payments Approval Notification')])

            channel_id.message_post(
            subject='Finance Approved a Payments',
            body='''Finance Approved a Payments & Approval Sent To CEO ''',
            subtype='mail.mt_comment')
    ...
    def second_approval_reject(self):
        try:
            users=self.env['res.users'].search([('active','=',True)])
            # ref = self.name
            users.notify_info("Finance Canceled a Payments,, ")

            channel_id = self.env['mail.channel'].search([('name','=','Payments Approval Notification')])

            channel_id.message_post(
            subject='Finance Canceled a Payments',
            body='''Finance Canceled a Payments''',
            subtype='mail.mt_comment')
    ...
    def third_approval(self):
        try:
            ...
            channel_id.message_post(
            subject='CEO Approved a Payments',
    ...
    def third_approval_reject(self):
        try:
            ...
            channel_id.message_post(
            subject='CEO Canceled a Payments',
